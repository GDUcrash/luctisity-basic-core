...
export default class SoundPlayerComponent extends CoreComponent {
    ...
    play (sound: number) {
        eventManager.out.emit(EVOUT_PLAY_SOUND, this, this.sounds[sound], 0);
    }

    playAndWait (sound: number) {
        eventManager.out.emit(EVOUT_PLAY_SOUND, this, this.sounds[sound], 1);
    }

    playAndLoop (sound: number) {
        eventManager.out.emit(EVOUT_PLAY_SOUND, this, this.sounds[sound], 2);
    }

    stop (sound: number) {
        eventManager.out.emit(EVOUT_PLAY_SOUND, this, this.sounds[sound], 3);
    }

    stopAllSounds () {
        eventManager.out.emit(EVOUT_STOP_ALL_SOUNDS, this);
    }
